...
namespace EguibarIT.Delegation.CMDlets
{
    ...
    [Cmdlet(VerbsCommon.Set, "AdAclPkiTemplateAdmin", ConfirmImpact = ConfirmImpact.Medium)]
    [OutputType(typeof(int))]
    public class PkiTemplateAdmin : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            if (
                EguibarIT.Delegation.Constructors.AclCon5.AclConstructor5(
                    string.Format("CN=Certificate Templates,CN=Public Key Services,CN=Services,{0}", EguibarIT.Delegation.Other.Domains.GetConfigurationNamingContext()),
                    _group,
            ...
            if (
                EguibarIT.Delegation.Constructors.AclCon5.AclConstructor5(
                    string.Format("CN=OID,CN=Public Key Services,CN=Services,{0}", EguibarIT.Delegation.Other.Domains.GetConfigurationNamingContext()),
                    _group,
                    ActiveDirectoryRights.GenericAll,
                    AccessControlType.Allow,
                    System.Guid.Empty,
            ...
            {
                WriteVerbose("Delegate PKI Template Administration completed succesfully.");
            }

            if (valueFromConstructor != 2)
            {
                WriteWarning("Something went wrong when delegating PKI Template Administration");
            }
        }
